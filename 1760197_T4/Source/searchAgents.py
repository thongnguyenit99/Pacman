...
def cornersHeuristic(state, problem):
    ...
    for i in range(4):  # vẫn còn nhiều nhất là 4 góc không mong muốn, vì vậy chúng ta cần phải đi trong bốn lần
        ...
        for cornerDiatance in cornersDistance:
            if (visitedlist[closestIndex]):  # Trường hợp góc thứ 1 đã ghé thăm  thì ta sẽ
                closestIndex = cornerIndex

            #  kiểm tra từng góc:
            #  1. không được truy cập đến  hay ghé thăm
            #  2. khoảng cách nhỏ hơn khoảng cách gần nhất hiện tại = > được đặt thành góc gần nhất mới
...
class FoodSearchProblem:
    ...
    def getCostOfActions(self, actions):
        ...
        for action in actions:
            ...
            # tìm ra trạng thái tiếp theo và xem liệu nó khả thi hay không
            dx, dy = Actions.directionToVector(action)
            x, y = int(x + dx), int(y + dy)
            if self.walls[x][y]:
                return 999999
