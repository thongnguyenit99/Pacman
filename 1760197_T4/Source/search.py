...
def uniformCostSearch(problem):
    ...
    "*** YOUR CODE HERE ***"
    # UCS là sd hàng đợi ưu tiên, sd Queue
    start = problem.getStartState()
    exploredState = []
        ...
        if state not in exploredState:
            ...
            for succ in successors:
                ...
                if coordinates not in exploredState:
                    directions = succ[1]
                    # chi phí đường đi mới sẽ bằng hành động tại điểm đó mà pacman có thể đi cộng hướng đi
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not states.isEmpty():
        ...
        if problem.isGoalState(state):
            return actions
# Nếu điểm mà k nằm trong cái khuông chương trình có các điểm khác xung quanh khi pacman đi tìm đường đi
        if state not in exploredState:
            # problem.getSuccessors(state) để mở trạng thái state, trả về 1 danh sách gồm hướng đi qua các list hành động, hướng đi
            successors = problem.getSuccessors(state)
            for succ in successors:
                # gán biến toạ độ
                coordinates = succ[0]
                if coordinates not in exploredState:
                    ...
                    # tại đây , mình sẽ tính chi phi đường đi tìm kiếm cho hành động mà pacman đi
